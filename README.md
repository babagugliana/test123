git checkout main (goes to main branch) <br>
git pull (pulls all changes from main branch) <br>
git checkout -b ALEX-01    (makes a new branch called ALEX-01) <br>
git push -u origin ALEX-01 (Syncs the new branch to the remote repo so it is up there too) <br>
git status (checks status / shows all files changed) <br>
git add -u (adds any currently changed file to your commit - you can add new files using "git add ::filename::") <br>
git commit -m "Fix: made a change to module X to allow picture upload" (commits changes to local branch) <br>
git push (pushes out changes to the ALEX-01 branch on gitlab.com so everyone can see them) <br>
